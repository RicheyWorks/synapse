...
<div class="flex flex-col gap-8 p-8 max-w-4xl mx-auto w-full">
  ...
  {:else}
    {#if gamification}
      <section class="rounded-xl border border-[var(--border)] bg-[var(--bg-elevated)] p-6 flex flex-col gap-3">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
          {#each gamification.achievements as a (a.id)}
            ...
            >
              <div class="text-sm font-medium text-[var(--text)]">{a.title}</div>
        ...
        <div class="flex flex-col gap-2">
          {#each hardest as item (item.id)}
            <div class="rounded-lg border border-[var(--border)] bg-[var(--bg-elevated)]">
              <button class="w-full flex items-center justify-between px-4 py-3 text-left" onclick={() => selectItem(item)}>
